...
void register_key_action(std::string keyname, std::string padname,bool inverse) {
  ...
  spdlog::info("registered {}({}) to {}", keyboard_layout_a75[keycode],keycode, padname);
}
...
{ 
    ...
    register_key_action(key, action, false);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (!VIGEM_SUCCESS(retval)) {
    spdlog::error("ViGEm Bus connection failed with error code: {}", uint64_t(retval));
    return -1;
  }
    ...
    if (res < 0) {
     ...
     /* receive_packet_controller(client, pad, hid_readbuffer);
      ...
      std::cout << "sThumbRY:  " << g_xinput_state.Gamepad.sThumbRY
                << "        ";
      conout.setpos(0, 14);
      std::cout << "bLeftTrigger:  " << int(g_xinput_state.Gamepad.bLeftTrigger)
                << "        ";
      conout.setpos(0, 15);
